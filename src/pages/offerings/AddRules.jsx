...
import { campaign_types, weekdays } from "../../utils/Common.js";
...
export default function AddRule() {
  ...
  const handleCheckboxChange = (id) => {
    let selectedDays = values.visibleOn ? values.visibleOn.split(",").map(Number) : [];

    if (selectedDays.includes(id)) {
      selectedDays = selectedDays.filter(dayId => dayId !== id); 
    } else {
      selectedDays.push(id); 
    }

    setFieldValue("visibleOn", selectedDays.join(","));
  };
  ...
  const {values, errors, touched, handleBlur, handleChange, setFieldValue, handleSubmit, isSubmitting} = useFormik({
    initialValues: {
      ...
      offerType:"1",
      displayTo:"0",
      visibleOn: weekdays.map(day => day.id).join(",") 
    },
                                        ...
                                        <div key={day.id} className="col-md-2 d-flex align-items-center">
                                          ...
                                          <div className="col-md-2">
                                            <input
                                              ...
                                              name={`day_${day.id}`}
                                              value={day.id}
                                              checked={values.visibleOn.split(",").includes(day.id.toString())}
